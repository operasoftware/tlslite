...
class TLSConnection(TLSRecordLayer):
	...
	def __init__(self, sock):
		...
		self._handshake_configuration = None
	...
	def _handshakeClientAsync(self, srpParams=(), certParams=(),
							 ...
							 session=None, settings=None, checker=None,
							 recursive=False, servername= None):

		self._handshake_configuration = {"srpParams":srpParams, "certParams":certParams, "unknownParams":unknownParams,
								   "sharedKeyParams":sharedKeyParams, "settings":settings, "checker":checker, "recursive":recursive, "servername":servername}

		handshaker = self._handshakeClientAsyncHelper(srpParams=srpParams,
				certParams=certParams, unknownParams=unknownParams,
				sharedKeyParams=sharedKeyParams, session=session,
				settings=settings, recursive=recursive, servername=servername)
		for result in self._handshakeWrapperAsync(handshaker, checker):
			yield result


	def _handshakeClientAsyncHelper(self, srpParams, certParams, unknownParams,
							   sharedKeyParams, session, settings, recursive, servername = None):
		if not recursive:
		...
		self.extra_padding_bytes = settings.extra_padding_bytes
		self.clientcertificate_requested = False
		...
		if settings.send_swapped_hello_versions:
			self.rec_version = settings.maxVersion
			self.version = (3,0)
		else:
			self.rec_version = settings.record_versions

		if settings.start_fake_renego_indication:
			renego_bytes = getRandomBytes(12)
		else:
			renego_bytes = 	self.our_verify_data
		if settings.use_fake_renego_indication and renego_bytes != None:
			renego_bytes[0]+=1
		...
		if session:
			...
			if (not sharedKeyParams) and \
				...
				clientHello.create(self.version, clientRandom,
								   session.sessionID, cipherSuites,
								   certificateTypes, session.srpUsername,
								   servername, settings.send_extensions,
							   	   settings.send_renego_extensions, 
							   	   renego_bytes,
							   	   send_certstatus= settings.send_extensions,
							   	   renegotiating = self.renegotiationStarted,
							   	   session_ticket= session.session_ticket,
							   	   renego_both = settings.send_renego_extension_and_scsv,
							   	   extra_action_set = settings.extra_action_set,
							   	   extra_action_dict = settings.extra_action_dict,
							   	   )
...
#			from tlscommon.ssl_v2_test import SSLv2_ClientHello
#			import errno
#			clientHello = SSLv2_ClientHello(settings)
#			bytes = clientHello.write()
...
#			s= bytesToString(concatArrays(createByteArraySequence([0x80 | ((len(bytes)>>8) &0x7f), len(bytes) & 0xff]), bytes))
#			while 1:
#				try:
...
#				yield 1
			...
			clientHello = ClientHello()
			clientHello.create(self.version, clientRandom,
							   getRandomBytes(32), cipherSuites,
		...
		if isinstance(msg, ServerHello):
			...
			if settings.start_fake_renego_indication:
				for result in self._sendError(AlertDescription.handshake_failure):
					yield result
				raise TLSLocalAlert(Alert(AlertDescription.handshake_failure + 2048))
		...
		self.detected_version = self.version
		self.received_session_ticket_ext = serverHello.session_ticket_ext_received
		...
		if serverHello.compression_method != 0:
			for result in self._sendError(\
				...
				yield result
		if serverHello.renego_error:
			for result in self._sendError(\
				AlertDescription.handshake_failure,
				"Server responded with incorrect renegotiation extension"):
				yield result
		if serverHello.support_renego:
			assert((self.our_verify_data and self.peer_verify_data) or (not self.our_verify_data and not self.peer_verify_data))
			if ((not self.our_verify_data and serverHello.renego_bytes) or
				 (serverHello.renego_bytes != (self.our_verify_data + self.peer_verify_data if self.our_verify_data else None))):
				for result in self._sendError(\
					AlertDescription.handshake_failure,
					"Server responded with incorrect renegotiation finished bytes"):
					yield result
				

		expect_certstatus = serverHello.cert_status_ext_received
		expect_session_ticket = serverHello.session_ticket_ext_received

		if session:
			if serverHello.support_renego:
				session.received_renego_information = True
			if serverHello.signal_SNI:
				session.received_SNI_signal = True
			if self.received_SNI_alert:
				session.received_SNI_alert =True
		...
		if session and session.sessionID and \
			...
			if self.received_session_ticket_ext:
				for result in self._getMsg(ContentType.handshake,
				...
				self.session.session_ticket = newSessionTicket.ticket 
		...
		#If server DOES NOT agree to resume
			...
			#If the server chose an RSA suite...
				...
				if isinstance(msg, CertificateRequest):
					...
					serverHelloDone = msg
			elif settings.just_test_cipher:
				dhe_keysize = 0
				if cipherSuite in CipherSuite.dheSuites:

					try:
						if cipherSuite not in CipherSuite.anonSuites:
							for result in self._getMsg(ContentType.handshake,
						...
						dhe_keysize = numBits(serverKeyExchange.dh_p)
					except Exception, problem:
						raise
				
				self.close()
				self.session = Session()
				self.session.test_cipher_selected = True
				self.session.dhe_keysize = dhe_keysize
			...
			if cipherSuite in CipherSuite.srpSuites + \
				...
				for result in self._sendMsg(\
						ClientKeyExchange(cipherSuite).createSRP(A), collect_records=True):
					yield result
			...
			#Calculate RSA premaster secret, if server chose an RSA suite
				...
				if settings.force_premaster_version:
					premasterSecret[0] = settings.force_premaster_version[0]
					premasterSecret[1] = settings.force_premaster_version[1]
				if settings.premaster_version_correct:
					...
					premasterSecret[0] = self.version[0]
					premasterSecret[1] = self.version[1]
				...
				if certificateRequest and (clientCertChain or (self.version > (3,0) and not settings.use_no_cert_alert)):
					...
					for result in self._sendMsg(clientCertificate):
						yield result
				elif certificateRequest : # implied empty cert
					assert(not clientCertChain)
					
					alertMsg = Alert()
					alertMsg.create(AlertDescription.no_certificate,
												AlertLevel.warning)
				...
				for result in self._sendMsg(clientKeyExchange, collect_records=True):
					yield result
				...
				if certificateRequest and privateKey:
					if self.version == (3,0):
						...
						verifyBytes = stringToBytes(self._handshake_sha256.digest())
			...
			if expect_certstatus:
				self.session.certificateStatusResponse = certificateStatus.ocspRepsonse
			if serverHello.support_renego:
				self.session.received_renego_information = True
			if serverHello.signal_SNI:
				self.session.received_SNI_signal = True
			if self.received_SNI_alert:
				self.session.received_SNI_alert =True
			...
			if self.received_session_ticket_ext:
				for result in self._getMsg(ContentType.handshake,
					...
					if result in (0,1) or isinstance(result, ApplicationData):
						...
						break
				newSessionTicket = result
				self.session.session_ticket = newSessionTicket.session_ticket
				if not self.session.sessionID:
					self.session.sessionID = getRandomBytes(32)
	...
	def _handshakeServerAsyncHelper(self, sharedKeyDB, verifierDB,
		...
		self._handshake_configuration = [sharedKeyDB, verifierDB,
							 certChain, privateKey, reqCert, sessionCache,
							 settings]
		...
		if clientHello.support_renego:
			assert((self.our_verify_data and self.peer_verify_data) or (not self.our_verify_data and not self.peer_verify_data))
			if ((not self.peer_verify_data and clientHello.renego_bytes) or
				 (clientHello.renego_bytes != self.peer_verify_data)):
				for result in self._sendError(\
					AlertDescription.handshake_failure,
					"Client initiated connection with incorrect renegotiation finished bytes"):
					yield result
				return;
		...
		if clientHello.session_id and (sharedKeyDB or sessionCache):
			...
			if session:
				...
				serverHello.create(self.version, serverRandom,
								   ...
								   certificateType,
								   clientHello.support_renego,
								   (self.peer_verify_data + self.our_verify_data if self.peer_verify_data else None)
								   )
		...
		if cipherSuite in CipherSuite.srpSuites + CipherSuite.srpRsaSuites:
			...
			if not clientHello.srp_username:
				...
				if clientHello.support_renego:
					assert((self.our_verify_data and self.peer_verify_data) or (not self.our_verify_data and not self.peer_verify_data))
					if ((not self.peer_verify_data and serverHello.renego_bytes) or
						 (serverHello.renego_bytes != self.peer_verify_data)):
						for result in self._sendError(\
							AlertDescription.handshake_failure,
							"Client initiated connection with incorrect renegotiation finished bytes"):
							yield result
	...
	def readAsync(self, max=None, min=1):
		...
		for result in TLSRecordLayer.readAsync(self, max, min):
			if isinstance(result, ClientHello):
				self.renegotiationStarted = True
				for result in self.handshakeServerAsync(*self._handshake_configuration):
					if result in (0,1) or isinstance(result, ApplicationData):
						yield result
			elif isinstance(result, HelloRequest):
				self.renegotiationStarted = True
				if self._handshake_configuration["settings"].refuse_renegotiation:
					alertMsg = Alert()
					...
					parameters =  self._handshake_configuration
					session = parameters.pop("session",self.session)
					settings = parameters.pop("settings")
					if settings.renegotiation_settings:
						settings = settings.renegotiation_settings
					settings = settings._filter()
					settings.maxVersion = self.version
					settings.record_versions= self.version
						
					if settings.renegotiation_new_session:
						session = None
					for result in self._handshakeClientAsync(session=session, settings = settings, **self._handshake_configuration):
						if result in (0,1) or isinstance(result, ApplicationData):
							yield result
		yield result
		
	def startRenegotiation(self, async=False, **parameters):
		"""Starts a renegotiation handshake with the specified handshake parametes
		Same API as _handshakeClientAsync
		
		Immediately return all Application data received just like read(); 
		use for result in foo.startRenegotiation to process the data
		
		If writes that are not syncronized with the arriving data are planned, use the async mode.
		"""
		
		if not parameters and self._handshake_configuration:
			parameters =  self._handshake_configuration
		...
		if self._client:
			...
			handshaker = self._handshakeClientAsync(session=session, settings = settings, **parameters)
		else:
			handshaker = self.handshakeServerAsync(*parameters)
