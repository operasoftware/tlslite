...
class ClientHello(HandshakeMsg):
    ...
    def create(self, version, random, session_id, cipher_suites,
               certificate_types=None, srp_username=None, servername = None, use_extensions = False):
    ...
    def write(self, trial=False):
        ...
        if self.use_extensions:
            extLength = 0
            if self.servername:
                extLength += 4 + len(self.servername)
            ...
            if self.servername:
                w.add(2+len(self.servername),2)
                w.add(0,1)
                w.addVarSeq(stringToBytes(self.servername),1,1)
